...
object TaskCompletion {
  ...
  private val codes = List(
       ...
       EnumValue("FAILURE_BAD_AUTHENTICATION", 4, "The outstation rejected the operation do to a lack of valid session keys or the user not existing on the outstation (SA only)"),
       EnumValue("FAILURE_NOT_AUTHORIZED", 5, "The outstation rejected the operation because the specified user is not authorized for the request that was made (SA only)")
  )
