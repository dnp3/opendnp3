...
TEST_CASE(SUITE("ResetLinks"))
{
	...
	REQUIRE(f.IsComplete());
	REQUIRE(ToHex(f) == "05 64 05 C0 01 00 00 04 E9 21");
}
...
TEST_CASE(SUITE("ACK"))
{
	...
	REQUIRE(ToHex(f) == "05 64 05 00 00 04 01 00 19 A6"); //ACK - Slave

	// ACK - Slave (DFC = true)
	f.FormatAck(false, true, 1024, 1);
	REQUIRE(ToHex(f) == "05 64 05 10 00 04 01 00 8B 0C"); // ACK - Slave (with DFC set)

	// ACK - Master (DFC = false)
	f.FormatAck(true, false, 1, 1024);
	REQUIRE(ToHex(f) == "05 64 05 80 01 00 00 04 53 11");

	// ACK - Master (DFC = true)
	f.FormatAck(true, true, 1, 1024);
	REQUIRE(ToHex(f) == "05 64 05 90 01 00 00 04 C1 BB");
}
...
TEST_CASE(SUITE("ConfirmedUserData"))
{
	...
	REQUIRE(ToHex(f) == "05 64 14 F3 01 00 00 04 0A 3B C0 C3 01 3C 02 06 3C 03 06 3C 04 06 3C 01 06 9A 12");
	...
	REQUIRE(ToHex(f) == "05 64 53 73 00 04 01 00 03 FC C1 E3 81 96 00 02 01 28 01 00 00 00 01 02 01 28 05 24 01 00 01 00 01 02 01 28 01 00 02 00 01 02 01 28 B4 77 01 00 03 00 01 20 02 28 01 00 00 00 01 00 00 20 A5 25 02 28 01 00 01 00 01 00 00 01 01 01 00 00 03 00 2F AC 00 1E 02 01 00 00 01 00 01 00 00 01 00 00 16 ED");
}
...
TEST_CASE(SUITE("ResetLinkStates"))
{
	...
	REQUIRE(ToHex(f) == "05 64 05 40 00 04 01 00 A3 96");
}
...
TEST_CASE(SUITE("LinkStatus"))
{
	...
	REQUIRE(ToHex(f) == RepairCRC("05 64 05 9B 01 00 00 04 E9 21"));
}
...
TEST_CASE(SUITE("NotSupported"))
{
	...
	REQUIRE(ToHex(f) == RepairCRC("05 64 05 1F 01 00 00 04 28 5A"));
}
