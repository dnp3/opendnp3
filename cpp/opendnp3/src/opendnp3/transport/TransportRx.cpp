	...
	sequence(0)
{
	if(fragSize > rxBuffer.Size())
	{
		...
		LOG_BLOCK(levels::ERR, "specified fragment size exceeds underling buffer size");
	}
}
...
void TransportRx::HandleReceive(const openpal::ReadOnlyBuffer& input)
{
	if (input.Size() < 2)
	{
		ERROR_BLOCK(levels::WARN, "Received tpdu with no payload, size: " << input.Size(), TLERR_NO_PAYLOAD);
	}
	else if (input.Size() > TL_MAX_TPDU_LENGTH)
	{
		ERROR_BLOCK(levels::WARN, "Illegal arg: " << input.Size() << " exceeds max tpdu size of " << TL_MAX_TPDU_LENGTH, TLERR_TOO_MUCH_DATA);
	...
	else
	{
		...
		uint8_t hdr = input[0];
		LOG_BLOCK(levels::INTERPRET, "<- " << TransportLayer::ToString(hdr));
		...
		if (this->ValidateHeader(first, last, seq, payloadLength))
		{
			...
			{
				ERROR_BLOCK(levels::WARN, "Exceeded the buffer size before a complete fragment was read", TLERR_BUFFER_FULL);
				numBytesRead = 0;
			}
...
bool TransportRx::ValidateHeader(bool fir, bool fin, uint8_t sequence_, uint32_t payloadSize)
{
	...
	if(fir)
	{
		...
		if(numBytesRead > 0)
		{
			...
			ERROR_BLOCK(levels::WARN, "FIR received mid-fragment, discarding: " << numBytesRead << "bytes", TLERR_NEW_FIR);
			numBytesRead = 0;
		}
	}
	else if(numBytesRead == 0)   //non-first packet with 0 prior bytes
	{
		ERROR_BLOCK(levels::WARN, "non-FIR packet with 0 prior bytes", TLERR_MESSAGE_WITHOUT_FIR);
		return false;
	}

	if(sequence_ != sequence)
	{
		ERROR_BLOCK(levels::WARN, "Ignoring bad sequence, got: " << sequence_ << " expected: " << sequence, TLERR_BAD_SEQUENCE);
		return false;
	}
