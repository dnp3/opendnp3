...
namespace opendnp3
{
ParseResult FreeFormatParser::ParseHeader(openpal::RSlice& buffer,
                                          ...
                                          IAPDUHandler* pHandler)
{
    ...
    FORMAT_LOGGER_BLOCK(pLogger, settings.Filters(), "%03u,%03u %s, %s, count: %u size: %u", record.group,
                        record.variation, GroupVariationToString(record.enumeration),
                        QualifierCodeToString(record.GetQualifierCode()), FREE_FORMAT_COUNT, FREE_FORMAT_SIZE);
