...
namespace opendnp3
{
	...
	public:
		...
		template <class T>
		static uint16_t WritePrefixed(HeaderWriter& writer, IEventCollection<T>& items, const DNP3Serializer<T>& serializer)
		{
	...
	private:
		...
		template <class T>
		class PrefixedEventWriter final : public IEventWriter<T>
		{
