...
DatabaseBuffers::DatabaseBuffers(const DatabaseSizes& dbSizes,
                                 StaticTypeBitField allowedClass0Types,
                                 IndexMode indexMode)
    : buffers(dbSizes), class0(allowedClass0Types), indexMode(indexMode)
{
}
...
bool DatabaseBuffers::Load(HeaderWriter& writer)
{
    ...
    LoadFun functions[NUM_TYPE] = {&DatabaseBuffers::LoadType<BinarySpec>,
...
template<>
StaticBinaryVariation DatabaseBuffers::CheckForPromotion<BinarySpec>(const Binary& value,
