...
bool Header::WriteTo(opendnp3::HeaderWriter& writer) const
{
    ...
    case (HeaderType::Ranged8):
        return writer.WriteRangeHeader<openpal::UInt8>(QualifierCode::UINT8_START_STOP, id, value.range8.start,
                                                       value.range8.stop);
    case (HeaderType::Ranged16):
        return writer.WriteRangeHeader<openpal::UInt16>(QualifierCode::UINT16_START_STOP, id, value.range16.start,
                                                        value.range16.stop);
