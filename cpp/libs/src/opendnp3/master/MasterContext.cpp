...
std::shared_ptr<IMasterTask> MContext::AddScan(openpal::TimeDuration period,
...
std::shared_ptr<IMasterTask> MContext::AddRangeScan(
    GroupVariationID gvId, uint16_t start, uint16_t stop, openpal::TimeDuration period, TaskConfig config)
{
...
void MContext::Write(const TimeAndInterval& value, uint16_t index, TaskConfig config)
{
    auto builder = [value, index](HeaderWriter& writer) -> bool {
        return writer.WriteSingleIndexedValue<UInt16, TimeAndInterval>(QualifierCode::UINT16_CNT_UINT16_INDEX,
