...
MasterSessionStack::MasterSessionStack(const openpal::Logger& logger,
                                       const std::shared_ptr<asiopal::Executor>& executor,
                                       const std::shared_ptr<opendnp3::ISOEHandler>& SOEHandler,
                                       const std::shared_ptr<opendnp3::IMasterApplication>& application,
                                       const std::shared_ptr<opendnp3::IMasterScheduler>& scheduler,
...
std::shared_ptr<IMasterScan> MasterSessionStack::AddScan(openpal::TimeDuration period,
...
std::shared_ptr<IMasterScan> MasterSessionStack::AddAllObjectsScan(GroupVariationID gvId,
                                                                   ...
                                                                   const TaskConfig& config)
{
    auto get = [self = shared_from_this(), gvId, period, config] {
        return self->context.AddAllObjectsScan(gvId, period, config);
    };
    return MasterScan::Create(executor->ReturnFrom<std::shared_ptr<IMasterTask>>(get), this->scheduler);
}
...
std::shared_ptr<IMasterScan> MasterSessionStack::AddClassScan(const ClassField& field,
                                                              ...
                                                              const TaskConfig& config)
{
    auto get = [self = shared_from_this(), field, period, config] {
        return self->context.AddClassScan(field, period, config);
    };
    return MasterScan::Create(executor->ReturnFrom<std::shared_ptr<IMasterTask>>(get), this->scheduler);
}

std::shared_ptr<IMasterScan> MasterSessionStack::AddRangeScan(
    GroupVariationID gvId, uint16_t start, uint16_t stop, openpal::TimeDuration period, const TaskConfig& config)
{
    auto get = [self = shared_from_this(), gvId, start, stop, period, config] {
        return self->context.AddRangeScan(gvId, start, stop, period, config);
    };
    return MasterScan::Create(executor->ReturnFrom<std::shared_ptr<IMasterTask>>(get), this->scheduler);
}
...
{
    auto action = [self = shared_from_this(), gvId, start, stop, config]() -> void {
        self->context.ScanRange(gvId, start, stop, config);
    };
    return executor->strand.post(action);
}
