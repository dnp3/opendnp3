/**
 * Licensed to Green Energy Corp (www.greenenergycorp.com) under one or
 * more contributor license agreements. See the NOTICE file distributed
 * with this work for additional information regarding copyright ownership.
 * Green Energy Corp licenses this file to you under the Apache License,
 * Version 2.0 (the "License"); you may not use this file except in
 * compliance with the License.  You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 *
 * This project was forked on 01/01/2013 by Automatak, LLC and modifications
 * may have been made to this file. Automatak, LLC licenses these modifications
 * to you under the terms of the License.
 */
#include "StaticResponseContext.h"

#include <openpal/serialization/Serialization.h>

#include "opendnp3/app/WriteConversions.h"

using namespace openpal;

namespace opendnp3
{

/*
StaticResponseContext::StaticResponseContext(Database& database) :
	pDatabase(&database)	
{}

IINField StaticResponseContext::ReadAll(const GroupVariationRecord& record)
{	
	if (record.enumeration == GroupVariation::Group60Var1)
	{
		IINField result;
		result |= QueueRange<Binary>(defaults.binary);				
		result |= QueueRange<DoubleBitBinary>(defaults.doubleBinary);
		result |= QueueRange<BinaryOutputStatus>(defaults.binaryOutputStatus);
		result |= QueueRange<Counter>(defaults.counter);
		result |= QueueRange<FrozenCounter>(defaults.frozenCounter);
		result |= QueueRange<Analog>(defaults.analog);
		result |= QueueRange<AnalogOutputStatus>(defaults.analogOutputStatus);		
		result |= QueueRange<TimeAndInterval>(defaults.timeAndInterval);
		return result;		
	}
	else
	{
		switch (record.group)
		{
			case(1) :
				return ReadRange(record, pDatabase->FullRange<Binary>());
			case(3) :
				return ReadRange(record, pDatabase->FullRange<DoubleBitBinary>());
			case(10) :
				return ReadRange(record, pDatabase->FullRange<BinaryOutputStatus>());
			case(20) :
				return ReadRange(record, pDatabase->FullRange<Counter>());
			case(21) :
				return ReadRange(record, pDatabase->FullRange<FrozenCounter>());
			case(30) :
				return ReadRange(record, pDatabase->FullRange<Analog>());
			case(40) :
				return ReadRange(record, pDatabase->FullRange<AnalogOutputStatus>());
			case(50) :
				return ReadRange(record, pDatabase->FullRange<TimeAndInterval>());
			default:
				return IINField(IINBit::FUNC_NOT_SUPPORTED);
		}
	}		

	return IINField::Empty();
}

IINField StaticResponseContext::ReadRange(const GroupVariationRecord& record, const Range& range)
{	
	switch (record.enumeration)
	{
		// Group 1
	case(GroupVariation::Group1Var0) :
		return QueueRange<Binary>(range, defaults.binary);
	case(GroupVariation::Group1Var2) :
		return QueueRange<Binary>(range, StaticBinaryVariation::Group1Var2);

	case(GroupVariation::Group3Var0) :
		return QueueRange<DoubleBitBinary>(range, defaults.doubleBinary);
	case(GroupVariation::Group3Var2) :
		return QueueRange<DoubleBitBinary>(range, StaticDoubleBinaryVariation::Group3Var2);

		// Group 10
	case(GroupVariation::Group10Var0) :
		return QueueRange<BinaryOutputStatus>(range, defaults.binaryOutputStatus);
	case(GroupVariation::Group10Var2) :
		return QueueRange<BinaryOutputStatus>(range, StaticBinaryOutputStatusVariation::Group10Var2);

		// Group 20
	case(GroupVariation::Group20Var0) :
		return QueueRange<Counter>(range, defaults.counter);
	case(GroupVariation::Group20Var1) :
		return QueueRange<Counter>(range, StaticCounterVariation::Group20Var1);
	case(GroupVariation::Group20Var2) :
		return QueueRange<Counter>(range, StaticCounterVariation::Group20Var2);
	case(GroupVariation::Group20Var5) :
		return QueueRange<Counter>(range, StaticCounterVariation::Group20Var5);
	case(GroupVariation::Group20Var6) :
		return QueueRange<Counter>(range, StaticCounterVariation::Group20Var6);

		// Group 21
	case(GroupVariation::Group21Var0) :
		return QueueRange<FrozenCounter>(range, defaults.frozenCounter);
	case(GroupVariation::Group21Var1) :
		return QueueRange<FrozenCounter>(range, StaticFrozenCounterVariation::Group21Var1);
	case(GroupVariation::Group21Var2) :
		return QueueRange<FrozenCounter>(range, StaticFrozenCounterVariation::Group21Var2);
	case(GroupVariation::Group21Var5) :
		return QueueRange<FrozenCounter>(range, StaticFrozenCounterVariation::Group21Var5);
	case(GroupVariation::Group21Var6) :
		return QueueRange<FrozenCounter>(range, StaticFrozenCounterVariation::Group21Var6);
	case(GroupVariation::Group21Var9) :
		return QueueRange<FrozenCounter>(range, StaticFrozenCounterVariation::Group21Var9);
	case(GroupVariation::Group21Var10) :
		return QueueRange<FrozenCounter>(range, StaticFrozenCounterVariation::Group21Var10);

		// Group 30
	case(GroupVariation::Group30Var0) :
		return QueueRange<Analog>(range, defaults.analog);
	case(GroupVariation::Group30Var1) :
		return QueueRange<Analog>(range, StaticAnalogVariation::Group30Var1);
	case(GroupVariation::Group30Var2) :
		return QueueRange<Analog>(range, StaticAnalogVariation::Group30Var2);
	case(GroupVariation::Group30Var3) :
		return QueueRange<Analog>(range, StaticAnalogVariation::Group30Var3);
	case(GroupVariation::Group30Var4) :
		return QueueRange<Analog>(range, StaticAnalogVariation::Group30Var4);
	case(GroupVariation::Group30Var5) :
		return QueueRange<Analog>(range, StaticAnalogVariation::Group30Var5);
	case(GroupVariation::Group30Var6) :
		return QueueRange<Analog>(range, StaticAnalogVariation::Group30Var6);

		// Group 40
	case(GroupVariation::Group40Var0) :
		return QueueRange<AnalogOutputStatus>(range, defaults.analogOutputStatus);
	case(GroupVariation::Group40Var1) :
		return QueueRange<AnalogOutputStatus>(range, StaticAnalogOutputStatusVariation::Group40Var1);
	case(GroupVariation::Group40Var2) :
		return QueueRange<AnalogOutputStatus>(range, StaticAnalogOutputStatusVariation::Group40Var2);
	case(GroupVariation::Group40Var3) :
		return QueueRange<AnalogOutputStatus>(range, StaticAnalogOutputStatusVariation::Group40Var3);
	case(GroupVariation::Group40Var4) :
		return QueueRange<AnalogOutputStatus>(range, StaticAnalogOutputStatusVariation::Group40Var4);

		// Group 50
	case(GroupVariation::Group50Var4) :
		return QueueRange<TimeAndInterval>(range, StaticTimeAndIntervalVariation::Group50Var4);

	default:
		return IINField(IINBit::FUNC_NOT_SUPPORTED);
	}	

	return IINField::Empty();
}

bool StaticResponseContext::Load(HeaderWriter& writer)
{
	switch (LoadStaticData(writer))
	{
	case(StaticLoadResult::EMPTY):
	case(StaticLoadResult::COMPLETED) :
		return true;
	case(StaticLoadResult::FULL) :
	default:
		return false;
	}
}

StaticLoadResult StaticResponseContext::LoadStaticData(HeaderWriter& writer)
{
	return StaticLoadResult::COMPLETED;
}

*/


}


