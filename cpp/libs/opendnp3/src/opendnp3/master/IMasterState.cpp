...
IMasterState* MasterStateWaitForResponse::OnResponse(MasterContext* pContext, const APDUResponseHeader& response, const openpal::ReadOnlyBuffer& objects)
{
	if (response.control.SEQ == pContext->solSeq)
	{
		...
		switch (result)
		{
			...
			default:				
				pContext->NotifyTaskCompletion((result == IMasterTask::Result::SUCCESS) ? TaskCompletion::SUCCESS : TaskCompletion::FAILURE_BAD_RESPONSE);
...
{	
	pContext->NotifyTaskCompletion(TaskCompletion::FAILURE_RESPONSE_TIMEOUT);
