...
TEST_CASE(SUITE("UnknownCodeIsEchoed"))
{
    ...
    // conformance requires IIN 2.2 to be set whenever the command status is not supported
    REQUIRE(t.lower->PopWriteAsHex()
            == "C0 81 80 04 0C 01 17 01 03 AA 01 01 00 00 00 01 00 00 00 04"); // 0x04 status ==
                                                                               // CommandStatus::NOT_SUPPORTED
}
...
TEST_CASE(SUITE("CROBWithQUSetReturnsNotSupported"))
{
    ...
    // Select group 12 Var 1, count = 1, index = 3, QU set
    t.SendToOutstation("C0 03 0C 01 17 01 03 51 01 01 00 00 00 01 00 00 00 00");
...
TEST_CASE(SUITE("SelectOperateSameSequenceNumber"))
{
    ...
    REQUIRE(t.lower->PopWriteAsHex()
            == "C0 81 80 00 0C 01 17 01 03 01 01 01 00 00 00 01 00 00 00 02"); // 0x02 no select
    t.OnTxReady();
}
...
TEST_CASE(SUITE("SelectOperateNonMatchingRequests"))
{
    ...
    REQUIRE(t.lower->PopWriteAsHex()
            == "C1 81 80 00 0C 01 17 01 04 01 01 01 00 00 00 01 00 00 00 02"); // 0x02 no select
    t.OnTxReady();
}
...
TEST_CASE(SUITE("SelectOperateGroup41Var1"))
{
    ...
    t.SendToOutstation("C0 03 29 01 17 01 03 00 00 00 00 00");
    REQUIRE(t.lower->PopWriteAsHex()
            == "C0 81 80 00 29 01 17 01 03 00 00 00 00 00"); // 0x00 status == CommandStatus::SUCCESS
    t.OnTxReady();
...
TEST_CASE(SUITE("SelectOperateGroup41Var3"))
{
    ...
    t.SendToOutstation("C0 03 29 03 17 01 01 00 00 C8 42 00");
    REQUIRE(t.lower->PopWriteAsHex()
            == "C0 81 80 00 29 03 17 01 01 00 00 C8 42 00"); // 0x00 status == CommandStatus::SUCCESS
    t.OnTxReady();

    // operate group 41 Var 3, count = 1, index = 1
    t.SendToOutstation("C1 04 29 03 17 01 01 00 00 C8 42 00");
    REQUIRE(t.lower->PopWriteAsHex()
            == "C1 81 80 00 29 03 17 01 01 00 00 C8 42 00"); // 0x00 status == CommandStatus::SUCCESS
    t.OnTxReady();
}
...
TEST_CASE(SUITE("SelectOperateGroup41Var4"))
{
    ...
    t.SendToOutstation("C0 03 29 04 17 01 01 00 00 00 00 00 00 59 40 00");
    REQUIRE(t.lower->PopWriteAsHex()
            == "C0 81 80 00 29 04 17 01 01 00 00 00 00 00 00 59 40 00"); // 0x00 status == CommandStatus::SUCCESS
    t.OnTxReady();

    // operate group 41 Var 4, count = 1, index = 1
    t.SendToOutstation("C1 04 29 04 17 01 01 00 00 00 00 00 00 59 40 00");
    REQUIRE(t.lower->PopWriteAsHex()
            == "C1 81 80 00 29 04 17 01 01 00 00 00 00 00 00 59 40 00"); // 0x00 status == CommandStatus::SUCCESS
    t.OnTxReady();
}
