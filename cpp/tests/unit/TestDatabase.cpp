...
template<class Spec>
void TestBufferForEvent(bool isEvent,
                        const typename Spec::meas_t& newVal,
                        DatabaseTestObject& test,
                        std::deque<Event<Spec>>& queue)
{
...
TEST_CASE(SUITE("BinaryNoChange"))
{
    ...
    TestBufferForEvent<BinarySpec>(false, Binary(false, ToUnderlying(BinaryQuality::RESTART)), t,
                                   t.buffer.binaryEvents);
}
...
TEST_CASE(SUITE("AnalogLastReportedChange"))
{
    ...
    TestBufferForEvent(true, Analog(6, ToUnderlying(AnalogQuality::RESTART)), t,
                       t.buffer.analogEvents); // change by 6, so 6 should get recorded
...
TEST_CASE(SUITE("CounterLastReportedChange"))
{
    ...
    TestBufferForEvent(true, Counter(6, ToUnderlying(CounterQuality::RESTART)), t,
                       t.buffer.counterEvents); // change by 6, so 6 should get recorded
    TestBufferForEvent(false, Counter(1, ToUnderlying(CounterQuality::RESTART)), t, t.buffer.counterEvents);
