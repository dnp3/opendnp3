...
public:
    ...
    virtual IINField ProcessHeader(const RangeHeader& header,
                                   const ICollection<Indexed<IINValue>>& values) override final
    {
    ...
    virtual IINField ProcessHeader(const RangeHeader& header,
                                   const ICollection<Indexed<DoubleBitBinary>>& values) override final
    {
        return ProcessAny(header, values, staticDoubleBinaries);
    }

    virtual IINField ProcessHeader(const RangeHeader& header,
                                   const ICollection<Indexed<BinaryOutputStatus>>& values) override final
    {
        return ProcessAny(header, values, staticControlStatii);
    }

    virtual IINField ProcessHeader(const RangeHeader& header,
                                   const ICollection<Indexed<Counter>>& values) override final
    {
        return ProcessAny(header, values, staticCounters);
    }

    virtual IINField ProcessHeader(const RangeHeader& header,
                                   const ICollection<Indexed<FrozenCounter>>& values) override final
    {
    ...
    virtual IINField ProcessHeader(const RangeHeader& header,
                                   const ICollection<Indexed<AnalogOutputStatus>>& values) override final
    {
        return ProcessAny(header, values, staticSetpointStatii);
    }

    virtual IINField ProcessHeader(const RangeHeader& header,
                                   const ICollection<Indexed<OctetString>>& values) override final
    {
        return ProcessAny(header, values, rangedOctets);
    }

    virtual IINField ProcessHeader(const PrefixHeader& header,
                                   const ICollection<Indexed<Binary>>& values) override final
    {
    ...
    virtual IINField ProcessHeader(const PrefixHeader& header,
                                   const ICollection<Indexed<Analog>>& values) override final
    {
    ...
    virtual IINField ProcessHeader(const PrefixHeader& header,
                                   const ICollection<Indexed<BinaryCommandEvent>>& values) override final
    {
        return this->ProcessAny(header, values, binaryCommandEvents);
    }

    virtual IINField ProcessHeader(const PrefixHeader& header,
                                   const ICollection<Indexed<AnalogCommandEvent>>& values) override final
    {
    ...
    virtual IINField ProcessHeader(const opendnp3::FreeFormatHeader& header,
                                   const Group120Var1& value,
                                   const openpal::RSlice& object) override final
    {
...
private:
    template<class T> IINField ProcessAny(const HeaderRecord& record, const ICollection<T>& meas, std::vector<T>& items)
    {
