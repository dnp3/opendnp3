...
std::shared_ptr<asiodnp3::IMasterSession> ExampleListenCallbacks::get_outstation_session(uint16_t address)
{
	...
	const auto iter = std::find_if(this->sessions.begin(), this->sessions.end(), [=](const auto& item) { return item.address == address; });
...
void ExampleListenCallbacks::OnFirstFrame(uint64_t sessionid, const opendnp3::LinkHeaderFields& header, asiodnp3::ISessionAcceptor& acceptor)
{
	std::lock_guard<std::mutex> lock(this->mutex);


	// do we already have a session with this outstation?
	const auto iter = std::find_if(this->sessions.begin(), this->sessions.end(), [&](const auto& item) { return item.address == header.src; });
	if(iter != this->sessions.end()) {

		std::cout << "Already connected to outstation w/ address " << header.src << ". Closing first connection." << std::endl;
	...
	const auto session = acceptor.AcceptSession(
			GetSessionName(header.src, sessionid),
			std::make_shared<ExampleSOEHandler>(header.src),
			std::make_shared<asiodnp3::DefaultMasterApplication>(),
			config
	);
