...
{
public:
	RouterRecord(const std::string& arPortName, std::shared_ptr<VtoRouter> apRouter, IVtoWriter* apWriter, uint8_t aVtoChannelId);

	std::string mPortName;
	std::shared_ptr<VtoRouter> mpRouter;
	IVtoWriter* mpWriter;
	uint8_t mVtoChannelId;
};
...
public:
	...
	void StopRouter(IVtoWriter* apWriter, uint8_t aVtoChannelId);
	void StopAllRoutersOnWriter(IVtoWriter* apWriter);

	RouterRecord GetRouterOnWriter(IVtoWriter* apWriter, uint8_t aVtoChannelId);
...
private:
	...
	RouterRecordVector::iterator Find(IVtoWriter* apWriter, uint8_t aVtoChannelId);
	RouterRecordVector::iterator Find(IVtoWriter* apWriter);

	Logger* GetSubLogger(const std::string& arId, uint8_t aVtoChannelId);
