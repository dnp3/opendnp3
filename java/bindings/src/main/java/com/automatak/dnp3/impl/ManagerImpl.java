...
class ManagerImpl implements DNP3Manager {
    ...
    @Override
    public synchronized Channel addTCPServer(String id, int levels, String endpoint, int port, ChannelListener listener) throws DNP3Exception
    {
        if(this.pointer == 0)
        {
            throw new DNP3Exception("Manager has been shutdown");
        ...
        long ptr = get_native_channel_tcp_server(this.pointer, id, levels, endpoint, port, listener);
    ...
    @Override
    public synchronized Channel addTLSServer(String id, int levels, String endpoint, int port, TLSConfig config, ChannelListener listener) throws DNP3Exception
    {
        if(this.pointer == 0)
        {
            throw new DNP3Exception("Manager has been shutdown");
        ...
        long ptr = get_native_channel_tls_server(this.pointer, id, levels, endpoint, port, config, listener);
    ...
    private native long get_native_channel_tcp_server(long nativePointer, String id, int level, String endpoint, int port, ChannelListener listener);
    ...
    private native long get_native_channel_tls_server(long nativePointer, String id, int level, String endpoint, int port, TLSConfig config, ChannelListener listener);
