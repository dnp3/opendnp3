...
{
	const auto env = JNI::GetEnv();
	const auto jcommand = Convert(env, command);
...
CommandStatus CommandHandlerAdapter::Operate(const ControlRelayOutputBlock& command, uint16_t index, OperateType opType)
{
	const auto env = JNI::GetEnv();
	const auto jcommand = Convert(env, command);
	const auto jopType = JCache::OperateType.fromType(env, static_cast<jint>(opType));
	const auto jstatus = JCache::CommandHandler.operateCROB(env, proxy, jcommand, index, jopType);
	return CommandStatusFromType(static_cast<uint8_t>(JCache::CommandStatus.toType(env, jstatus)));
}
...
CommandStatus CommandHandlerAdapter::Select(const AnalogOutputInt16& command, uint16_t index)
{
	const auto env = JNI::GetEnv();
	const auto jcommand = Convert(env, command);
	const auto jstatus = JCache::CommandHandler.selectAOI16(env, proxy, jcommand, index);
	return CommandStatusFromType(static_cast<uint8_t>(JCache::CommandStatus.toType(env, jstatus)));
}
...
CommandStatus CommandHandlerAdapter::Operate(const AnalogOutputInt16& command, uint16_t index, OperateType opType)
{
	...
	const auto jcommand = Convert(env, command);
	const auto jopType = JCache::OperateType.fromType(env, static_cast<jint>(opType));
	const auto jstatus = JCache::CommandHandler.operateAOI16(env, proxy, jcommand, index, jopType);
	return CommandStatusFromType(static_cast<uint8_t>(JCache::CommandStatus.toType(env, jstatus)));
}
...
CommandStatus CommandHandlerAdapter::Select(const AnalogOutputInt32& command, uint16_t index)
{
	const auto env = JNI::GetEnv();
	const auto jcommand = Convert(env, command);
	const auto jstatus = JCache::CommandHandler.selectAOI32(env, proxy, jcommand, index);
	return CommandStatusFromType(static_cast<uint8_t>(JCache::CommandStatus.toType(env, jstatus)));
}
...
CommandStatus CommandHandlerAdapter::Operate(const AnalogOutputInt32& command, uint16_t index, OperateType opType)
{
	...
	const auto jcommand = Convert(env, command);
	const auto jopType = JCache::OperateType.fromType(env, static_cast<jint>(opType));
	const auto jstatus = JCache::CommandHandler.operateAOI32(env, proxy, jcommand, index, jopType);
	return CommandStatusFromType(static_cast<uint8_t>(JCache::CommandStatus.toType(env, jstatus)));
}
...
CommandStatus CommandHandlerAdapter::Select(const AnalogOutputFloat32& command, uint16_t index)
{
	const auto env = JNI::GetEnv();
	const auto jcommand = Convert(env, command);
	const auto jstatus = JCache::CommandHandler.selectAOF32(env, proxy, jcommand, index);
	return CommandStatusFromType(static_cast<uint8_t>(JCache::CommandStatus.toType(env, jstatus)));
}
...
CommandStatus CommandHandlerAdapter::Operate(const AnalogOutputFloat32& command, uint16_t index, OperateType opType)
{
	...
	const auto jcommand = Convert(env, command);
	const auto jopType = JCache::OperateType.fromType(env, static_cast<jint>(opType));
	const auto jstatus = JCache::CommandHandler.operateAOF32(env, proxy, jcommand, index, jopType);
	return CommandStatusFromType(static_cast<uint8_t>(JCache::CommandStatus.toType(env, jstatus)));
}
...
CommandStatus CommandHandlerAdapter::Operate(const AnalogOutputDouble64& command, uint16_t index, OperateType opType)
{
	...
	const auto jcommand = Convert(env, command);
	const auto jopType = JCache::OperateType.fromType(env, static_cast<jint>(opType));
	const auto jstatus = JCache::CommandHandler.operateAOD64(env, proxy, jcommand, index, jopType);
	return CommandStatusFromType(static_cast<uint8_t>(JCache::CommandStatus.toType(env, jstatus)));
}

jobject CommandHandlerAdapter::Convert(JNIEnv* env, const ControlRelayOutputBlock& command)
{
	const auto jcontrolcode = JCache::ControlCode.fromType(env, command.rawCode);
...
jobject CommandHandlerAdapter::Convert(JNIEnv* env, const AnalogOutputInt16& command)
{
	const auto jcommandstatus = JCache::CommandStatus.fromType(env, CommandStatusToType(command.status));
	return JCache::AnalogOutputInt16.init2(env, command.value, jcommandstatus);
}

jobject CommandHandlerAdapter::Convert(JNIEnv* env, const AnalogOutputInt32& command)
{
	const auto jcommandstatus = JCache::CommandStatus.fromType(env, CommandStatusToType(command.status));
	return JCache::AnalogOutputInt32.init2(env, command.value, jcommandstatus);
}

jobject CommandHandlerAdapter::Convert(JNIEnv* env, const AnalogOutputFloat32& command)
{
	const auto jcommandstatus = JCache::CommandStatus.fromType(env, CommandStatusToType(command.status));
	return JCache::AnalogOutputFloat32.init2(env, command.value, jcommandstatus);
}

jobject CommandHandlerAdapter::Convert(JNIEnv* env, const AnalogOutputDouble64& command)
{
	const auto jcommandstatus = JCache::CommandStatus.fromType(env, CommandStatusToType(command.status));
